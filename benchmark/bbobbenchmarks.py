        ...
        if zerof:
            ...
            self.fopt = min(
                ...
                max(
                    -1000,
                    (
                        np.round(100 * 100 * gauss(1, self.rseed)[0] / gauss(1, self.rseed + 1)[0])
                        / 100
                    ),
                ),
            )
...
class F16(BBOBNfreeFunction):
    ...
    def _evalfull(self, x):
        ...
        if len(curshape) < 2:  # popsize is one
            ftrue = np.sum(
                dot(
                    self.aK, np.cos(dot(self.bK.T, 2 * np.pi * (np.reshape(x, (1, len(x))) + 0.5)))
                )
            )
        else:
            ...
            for k, i in enumerate(x):
                ...
                ftrue[k] = np.sum(
                    dot(
                        self.aK,
                        np.cos(dot(self.bK.T, 2 * np.pi * (np.reshape(i, (1, len(i))) + 0.5))),
                    )
                )
    ...
    def initwithsize(self, curshape, dim):
        ...
        if self.dim != dim:
            ...
            self.peakvalues = np.insert(
                linspace(self.fitvalues[0], self.fitvalues[1], self.nhighpeaks - 1), 0, 10.0
            )
        ...
        if self.lastshape != curshape:
            ...
            self.xlocal = dot(
                self.fac2
                * np.reshape(
                    10.0 * unif(dim * self.nhighpeaks, self.rseed) - 5.0, (self.nhighpeaks, dim)
                ),
                self.rotation,
            )
            if self.zerox:
