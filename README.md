...
http请求地址是: `http://207.246.97.250:7200` (请注意一定要使用*7200*端口)

### 初始化一个新任务

发出`HTTP POST`请求：`http://207.246.97.250:7200`，其附带`JSON`数据字段, 例如:
...
这个请求，若无运行错误，会返回新创建的任务ID，例如：

```{json}
{
        "job_id" : "0r8FwcNmnXzRrPB"
}
```

在之后的请求中，用户必须使用所返回的`job_id`.

### 请求一组新的解

发出`HTTP GET`请求：`http://207.246.97.250:7200?ask='null'&job_id=0r8FwcNmnXzRrPB`，其通过一个`JSON`数据字段来返回这组被请求的解，例如：
...
用户需要执行在上一步中得到的这组解，并将运行后所得到的目标值加入到这个字段中，例如：
...
在这之后我们就可以发出`HTTP POST`请求：`http://207.246.97.250:7200`，其把如上所示的`JSON`作为数据字段。

### 终止一个任务

当用户不再想继续使用当前优化任务，或者需要创建新的优化任务时，该用户、一般的、应该终止掉当前的优化任务，这可通过以下
`HTTP GET`请求实现：`http://207.246.97.250:7200?finalize='null'&job_id=0r8FwcNmnXzRrPB`
