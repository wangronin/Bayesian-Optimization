...
发出`HTTP GET`请求：`http://127.0.0.1:7200?ask='null'&job_id=0r8FwcNmnXzRrPB`，其通过一个`JSON`数据字段来返回这组被请求的解，例如：
...
在这之后我们就可以发出`HTTP POST`请求：`http://127.0.0.1:7200`，其把如上所示的`JSON`作为数据字段。
