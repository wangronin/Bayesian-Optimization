...
class RemoteBO(BaseHTTPRequestHandler, object):
    ...
    def _get_history(self, data, rsp_data):
        ...
        for job_id in job_ids:
            dump_file = self._get_dump_file(job_id)
            opt = ParallelBO.load(dump_file)
            rsp_data[job_id] = {"hist_f": opt.hist_f if hasattr(opt, "hist_f") else None}
    ...
    def _get_feature_importance(self, data, rsp_data):
        ...
        for job_id in job_ids:
            ...
            rsp_data[job_id] = {"feature": feature_name, "imp": imp_.tolist()}
...
class RemoteBODaemon(Daemon):
    ...
    def run(self, host, port, verbose, work_dir, log_file):
        ...
        work_dir = (
            os.path.join(os.path.expanduser("~"), "BO-" + str(port))
            if work_dir is None
            else work_dir
        )
...
if __name__ == "__main__":
    ...
    opt_parser.add_option(
        "-p", "--port", action="store", dest="port", help="port number", default=7200, type="int"
    )
    ...
    opt_parser.add_option(
        ...
        action="store_true",
        dest="verbose",
        help="foreground/background mode",
        default=False,
    )
