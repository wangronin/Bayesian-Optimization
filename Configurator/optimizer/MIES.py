...
class MIES(object):
    ...
    def __init__(self, obj_func, x0, bounds, param_type, max_eval,
        ...
        fitness0 = self.obj_func(np.array(x0))
    ...
    def keep_in_bound(self, pop):
        ...
        pop.iloc[:, self.id_d] = pop.iloc[:, self.id_d].applymap(int)
        return pop
    ...
    def _mutate_r(self, individual):
        sigma = individual[self.cols_sigma].values
    ...
    def _mutate_i(self, individual):
        eta = individual[self.cols_eta].values
    ...
    def _mutate_d(self, individual):
        P = individual[self.cols_p].values
...
if __name__ == '__main__':
    ...
    np.random.seed(1)
    def fitness(x):
        x_r, x_i, x_d = x[:2], x[2], x[3]
        return np.sum(x_r ** 2) + abs(x_i - 10) / 123. + abs(x_d - 1) * 2

    x0 = [2, 1, 80, 10]
    bounds = [[-5] * 2 + [-100, 0], [5] * 2 + [100, 10]]

    opt = MIES(fitness, x0, bounds, ['R', 'R', 'I', 'D'], 1e3)
    print opt.optimize()
